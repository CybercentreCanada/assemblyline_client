...
def test_alert(datastore, client):
    ...
    assert len(res) > 0
    assert res == sorted(list(datastore.alert.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])


def test_file(datastore, client):
    res = sorted([x for x in client.search.stream.file("id:*")], key=lambda k: k['id'])
    assert len(res) > 0
    assert res == sorted(list(datastore.file.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])


def test_heuristic(datastore, client):
    res = sorted([x for x in client.search.stream.heuristic("id:*")], key=lambda k: k['id'])
    assert len(res) > 0
    assert res == sorted(list(datastore.heuristic.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])
...
def test_signature(datastore, client):
    ...
    assert len(res) > 0
    assert res == sorted(list(datastore.signature.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])


def test_submission(datastore, client):
    res = sorted([x for x in client.search.stream.submission("id:*")], key=lambda k: k['id'])
    assert len(res) > 0
    assert res == sorted(list(datastore.submission.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])


def test_workflow(datastore, client):
    res = sorted([x for x in client.search.stream.workflow("id:*")], key=lambda k: k['id'])
    assert len(res) > 0
    assert res == sorted(list(datastore.workflow.stream_search("id:*", as_obj=False)), key=lambda k: k['id'])
