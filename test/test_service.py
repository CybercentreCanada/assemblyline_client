...
def test_restore_backup(datastore, client):
    original_data = datastore.list_all_services(full=True, as_obj=False)
    original_data.sort(key=lambda x: x["name"])

    original_service_versions = set(datastore.service.keys("id:*"))

    backup = client.service.backup(full=True)
    ...
    restored_service_versions = set(datastore.service.keys("id:*"))

    restored_data = datastore.list_all_services(full=True, as_obj=False)
    restored_data.sort(key=lambda x: x["name"])

    assert original_data == restored_data
    assert original_service_versions == restored_service_versions
