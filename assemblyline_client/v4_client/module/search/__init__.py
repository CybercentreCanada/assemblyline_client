...
"""
    ...
    def retrohunt(self, query, filters=None, fl=None, offset=0, rows=25, sort=None, timeout=None,
                  use_archive=False, track_total_hits=None):
        """\
Search retrohunt queries with a lucene query.
...
Returns all results.
"""
        return self._do_search('retrohunt', query, filters=filters, fl=fl, offset=offset,
                               rows=rows, sort=sort, timeout=timeout,
                               use_archive=use_archive, track_total_hits=track_total_hits)
